...
export const generateFinancialAnalysis = async (query: string): Promise<string> => {
  try {
    ...
    if (asset.includes('/')) {
      ...
      const response = await fetch(`https://finnhub.io/api/v1/forex/rates?base=${baseCurrency}&token=${process.env.FINNHUB_API_KEY || 'demo'}`);
      ...
      try {
        const response = await fetch(`https://financialmodelingprep.com/api/v3/profile/${asset}?apikey=${process.env.FMP_API_KEY || 'demo'}`);
